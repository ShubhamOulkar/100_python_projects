...
for number in nums:
    goal_positions[number] = find_location(rows_goal, str(number))


class FifteenPuzzleProblem(SearchProblem):
...
result = astar(FifteenPuzzleProblem(INITIAL))
# if you want to use the visual debugger, use this instead:
#result = astar(FifteenPuzzleProblem(INITIAL), viewer=WebViewer())
